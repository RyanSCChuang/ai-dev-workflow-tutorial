...
- **Validation extended after code review.** `load_sales` also raises `ValueError` for blank or unparsable dates (which would otherwise become `NaT` and be dropped from the monthly trend, so the chart could disagree with the KPIs) and for non-numeric `quantity`, `unit_price` or `total_amount`. Each shows the friendly red message. The zero-byte-file branch is now covered by a test.
- **Tooltips show exact dollars and cents.** The three charts use `$%{...:,.2f}` to match the PRD's "exact values" (FR-2 to FR-4). Only the Total Sales KPI card stays in whole dollars, as decided.
